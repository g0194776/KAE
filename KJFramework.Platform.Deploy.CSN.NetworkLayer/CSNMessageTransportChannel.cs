...
namespace KJFramework.Platform.Deploy.CSN.NetworkLayer
{
    ...
    public class CSNMessageTransportChannel<T> : IMessageTransportChannel<T>
    {
        #region Constructor
        ...
        public CSNMessageTransportChannel(ICSNRawTransportChannel rawChannel, IProtocolStack<T> protocolStack, ICSNSegmentDataParser<T> parser)
        {
            ...
            else
            {
                ...
                if (_rawChannel.Buffer == null) _rawChannel.Buffer = new ReceiveBuffer(CSNChannelConst.RecvBufferSize);
            }
        ...
        public int Send(T obj)
        {
            ...
            if (data.Length <= CSNChannelConst.MaxMessageDataLength) return Send(data);
            //need multi messages transport.
            List<byte[]> multiDatas = _protocolStack.ConvertMultiMessage(obj, CSNChannelConst.MaxMessageDataLength);
        ...
        void ReceivedDataSegment(object sender, CSNSegmentReceiveEventArgs e)
        {
            _parser.Append(e);
        }
