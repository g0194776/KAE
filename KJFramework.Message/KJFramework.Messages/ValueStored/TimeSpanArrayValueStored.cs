...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     TimeSpan数组类型的存储
    /// </summary>
    public class TimeSpanArrayValueStored : BaseValueStored
    {
        #region Members
        ...
        /// </summary>
        public TimeSpanArrayValueStored()
        {
            _typeId = (byte)PropertyTypes.TimeSpanArray;
        }

        /// <summary>
        ///     TimeSpan数组类型存储的初始化构造器
        /// </summary>
        public TimeSpanArrayValueStored(TimeSpan[] value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.TimeSpanArray;
        ...
        /// </summary>
        static TimeSpanArrayValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<TimeSpan[]>();
            _toBytesDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.TimeSpanArray)).ValueProcessor;
            _toDataDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.TimeSpanArray)).DataProcessor;
        }
        ...
        {
            return new TimeSpanArrayValueStored();
        }
