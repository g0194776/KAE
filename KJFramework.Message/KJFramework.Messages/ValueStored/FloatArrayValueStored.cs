...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     Float数组类型的存储
    /// </summary>
    public class FloatArrayValueStored : BaseValueStored
    {
        #region Members
        ...
        ///     Float数组类型存储的初始化构造器
        /// </summary>
        public FloatArrayValueStored()
        {
            _typeId = (byte)PropertyTypes.FloatArray;
        }

        /// <summary>
        ///     Float数组类型存储的初始化构造器
        /// </summary>
        public FloatArrayValueStored(float[] value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.FloatArray;
        ...
        ///     Float数组类型存储的静态构造器
        /// </summary>
        static FloatArrayValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<float[]>();
            _toBytesDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.FloatArray)).ValueProcessor;
            _toDataDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.FloatArray)).DataProcessor;
        }
        ...
        {
            return new FloatArrayValueStored();
        }
