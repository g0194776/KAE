...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     Double类型的存储
    /// </summary>
    public class DoubleValueStored : BaseValueStored
    {
        #region Members
        ...
        ///     Double类型存储的初始化构造器
        /// </summary>
        public DoubleValueStored()
        {
            _typeId = (byte)PropertyTypes.Double;
        }

        /// <summary>
        ///     Double类型存储的初始化构造器
        /// </summary>
        public DoubleValueStored(double value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.Double;
        }
        ...
        ///     Double类型存储的静态构造器
        /// </summary>
        static DoubleValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<double>();
            _toBytesDelegate = ProcessorDictionary.ValueActions[(byte)PropertyTypes.Double];
            _toDataDelegate = ProcessorDictionary.DataActions[(byte)PropertyTypes.Double];
        }
