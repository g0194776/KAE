...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     Byte类型的存储
    /// </summary>
    public class ByteValueStored : BaseValueStored
    {
        #region Members
        ...
        ///     Byte类型存储的初始化构造器
        /// </summary>
        public ByteValueStored()
        {
            _typeId = (byte)PropertyTypes.Byte;
        }

        /// <summary>
        ///     Byte类型存储的初始化构造器
        /// </summary>
        public ByteValueStored(byte value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.Byte;
        }
        ...
        ///     Byte类型存储的静态构造器
        /// </summary>
        static ByteValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<byte>();
            _toBytesDelegate = ProcessorDictionary.ValueActions[(byte) PropertyTypes.Byte];
            _toDataDelegate = ProcessorDictionary.DataActions[(byte)PropertyTypes.Byte];
        }
