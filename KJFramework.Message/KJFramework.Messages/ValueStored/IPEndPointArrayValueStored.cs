...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     IPEndPoint数组类型的存储
    /// </summary>
    public class IPEndPointArrayValueStored : BaseValueStored
    {
        #region Members

        private readonly IPEndPoint[] _value;
        private static readonly PropertyValueStored<IPEndPoint[]> _instance;
        ...
        /// </summary>
        public IPEndPointArrayValueStored()
        {
            _typeId = (byte)PropertyTypes.IPEndPointArray;
        }

        /// <summary>
        ///     IPEndPoint数组类型存储的初始化构造器
        /// </summary>
        public IPEndPointArrayValueStored(IPEndPoint[] value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.IPEndPointArray;
        ...
        /// </summary>
        static IPEndPointArrayValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<IPEndPoint[]>();
            _toBytesDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.IPEndPointArray)).ValueProcessor;
            _toDataDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.IPEndPointArray)).DataProcessor;
        }
        ...
        {
            return new IPEndPointArrayValueStored();
        }
