...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     UInt16数组类型的存储
    /// </summary>
    public class UInt16ArrayValueStored : BaseValueStored
    {
        #region Members
        ...
        ///     UInt16数组类型存储的初始化构造器
        /// </summary>
        public UInt16ArrayValueStored()
        {
            _typeId = (byte)PropertyTypes.UInt16Array;
        }

        /// <summary>
        ///     UInt16数组类型存储的初始化构造器
        /// </summary>
        public UInt16ArrayValueStored(ushort[] value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.UInt16Array;
        ...
        ///     UInt16数组类型存储的静态构造器
        /// </summary>
        static UInt16ArrayValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<ushort[]>();
            _toBytesDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.UInt16Array)).ValueProcessor;
            _toDataDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.UInt16Array)).DataProcessor;
        }
        ...
        {
            return new UInt16ArrayValueStored();
        }
