...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     Double数组类型的存储
    /// </summary>
    public class DoubleArrayValueStored : BaseValueStored
    {
        #region Members

        private readonly double[] _value;
        private static readonly PropertyValueStored<double[]> _instance;
        ...
        ///     Double数组类型存储的初始化构造器
        /// </summary>
        public DoubleArrayValueStored()
        {
            _typeId = (byte)PropertyTypes.DoubleArray;
        }

        /// <summary>
        ///     Double数组类型存储的初始化构造器
        /// </summary>
        public DoubleArrayValueStored(double[] value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.DoubleArray;
        ...
        ///     Double数组类型存储的静态构造器
        /// </summary>
        static DoubleArrayValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<double[]>();
            _toBytesDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.DoubleArray)).ValueProcessor;
            _toDataDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.DoubleArray)).DataProcessor;
        }
        ...
        {
            return new DoubleArrayValueStored();
        }
