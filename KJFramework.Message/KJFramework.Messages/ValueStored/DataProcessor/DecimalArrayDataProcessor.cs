...
namespace KJFramework.Messages.ValueStored.DataProcessor
{
    ...
    ///     Decimal数组关于元数据的处理
    /// </summary>
    public class DecimalArrayDataProcessor : IDataProcessor
    {
        #region Members
        ...
        {
            get { return PropertyTypes.DecimalArray; }
        }
        ...
        public unsafe void ValueProcessor(IMemorySegmentProxy proxy, BaseValueStored baseValueMessage)
        {
            ...
            fixed (decimal* pByte = baseValueMessage.GetValue<decimal[]>())
                proxy.WriteMemory(pByte, (uint)baseValueMessage.GetValue<decimal[]>().Length * Size.Decimal);
        }
