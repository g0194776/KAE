...
namespace KJFramework.Messages.ValueStored.DataProcessor
{
    ...
    ///     Guid数组关于元数据的处理
    /// </summary>
    public class GuidArrayDataProcessor : IDataProcessor
    {
        #region Members
        ...
        {
            get { return PropertyTypes.GuidArray; }
        }
        ...
        public unsafe void DataProcessor(MetadataContainer metadataObject, byte id, byte[] data, int offsetStart, uint length)
        {
            ...
            if (length == 0)
            {
                metadataObject.SetAttribute(id, new GuidArrayValueStored(new Guid[0]));
                return;
            }
            Guid[] array = new Guid[length / Size.Guid];
            fixed (byte* pByte = (&data[offsetStart]))
            {
                Guid* pData = (Guid*)pByte;
                for (int j = 0; j < array.Length; j++) array[j] = *pData++;
            }
            metadataObject.SetAttribute(id, new GuidArrayValueStored(array));
        }
        ...
        public unsafe void ValueProcessor(IMemorySegmentProxy proxy, BaseValueStored baseValueMessage)
        {
            ...
            fixed (Guid* pByte = baseValueMessage.GetValue<Guid[]>())
                proxy.WriteMemory(pByte, (uint)baseValueMessage.GetValue<Guid[]>().Length * Size.Guid);
        }
