...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    public class DecimalArrayIntellectTypeProcessor : IntellectTypeProcessor
    {
        ...
        public override byte[] Process(IntellectPropertyAttribute attribute, object value)
        {
            ...
            decimal[] arr = (decimal[])value;
            //id(1) + total length(4) + rank(4)
            memory = new byte[9 + Size.Decimal*arr.Length];
            ...
            unsafe
            {
                fixed (decimal* pInt = arr)
                {
                    fixed (byte* pData = &memory[offset])
                    {
                        decimal* pTemp = pInt;
                        decimal* pArray = (decimal*)pData;
        ...
        {
            decimal[] ret;
            if (length == 4) return new decimal[0];
            unsafe
            {
                fixed (byte* pByte = &data[offset])
                {
                    int arrLength = *(int*)pByte;
                    decimal* pTemp = (decimal*)(pByte + 4);
                    ret = new decimal[arrLength];
