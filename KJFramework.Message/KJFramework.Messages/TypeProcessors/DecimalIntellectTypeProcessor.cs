...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    /// </summary>
    public class DecimalIntellectTypeProcessor : IntellectTypeProcessor
    {
        #region 构造函数
        ...
        ///     Decimal类型智能处理器，提供了相关的基本操作。
        /// </summary>
        public DecimalIntellectTypeProcessor()
        {
        ...
        public override void Process(byte[] memory, int offset, IntellectPropertyAttribute attribute, object value)
        {
            unsafe
            {
                fixed (byte* pByte = &memory[offset]) *(decimal*)pByte = (decimal)value;
            }
        }
        ...
        {
            decimal value;
            if (!isNullable) value = analyseResult.GetValue<decimal>(target);
            else
            {
            ...
            if (attribute.AllowDefaultNull && value.Equals(DefaultValue.Decimal) && !isArrayElement) return;
        ...
        public override byte[] Process(IntellectPropertyAttribute attribute, object value)
        {
            ...
            unsafe
            {
                ...
                fixed (byte* pByte = data) *(decimal*)pByte = (decimal)value;
                return data;
            }
        }
        ...
        public override object Process(IntellectPropertyAttribute attribute, byte[] data, int offset, int length = 0)
        {
            ...
            unsafe
            {
                fixed (byte* pByte = &data[offset]) return *(decimal*)pByte;
            }
        }
