...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    public class DateTimeArrayIntellectTypeProcessor : IntellectTypeProcessor
    {
        ...
        public override byte[] Process(IntellectPropertyAttribute attribute, object value)
        {
            ...
            DateTime[] arr = (DateTime[])value;
            //id(1) + total length(4) + rank(4)
            memory = new byte[9 + Size.DateTime*arr.Length];
            ...
            unsafe
            {
                fixed (DateTime* pInt = arr)
                {
                    fixed (byte* pData = &memory[offset])
                    {
                        DateTime* pTemp = pInt;
                        DateTime* pArray = (DateTime*)pData;
        ...
        {
            DateTime[] ret;
            if (length == 4) return new DateTime[0];
            unsafe
            {
                fixed (byte* pByte = &data[offset])
                {
                    int arrLength = *(int*)pByte;
                    DateTime* pTemp = (DateTime*)(pByte + 4);
                    ret = new DateTime[arrLength];
