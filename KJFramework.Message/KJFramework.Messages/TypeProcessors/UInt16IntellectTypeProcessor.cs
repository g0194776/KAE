...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    /// </summary>
    public class UInt16IntellectTypeProcessor : IntellectTypeProcessor
    {
        #region 构造函数
        ...
        ///     UInt16类型智能处理器，提供了相关的基本操作。
        /// </summary>
        public UInt16IntellectTypeProcessor()
        {
        ...
        {
            BitConvertHelper.GetBytes((short)(ushort)value, memory, offset);
        }
        ...
        {
            ushort value;
            if (!isNullable) value = analyseResult.GetValue<ushort>(target);
            else
            {
            ...
            if (attribute.AllowDefaultNull && value.Equals(DefaultValue.UInt16) && !isArrayElement) return;
        ...
        {
            if(result.Nullable) result.SetValue<ushort?>(instance, BitConverter.ToUInt16(data, offset));
            else result.SetValue(instance, BitConverter.ToUInt16(data, offset));
        }
