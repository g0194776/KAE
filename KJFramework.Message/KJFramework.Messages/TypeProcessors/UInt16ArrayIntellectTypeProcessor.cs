...
namespace KJFramework.Messages.TypeProcessors
{
    ...
    public class UInt16ArrayIntellectTypeProcessor : IntellectTypeProcessor
    {
        ...
        public override byte[] Process(IntellectPropertyAttribute attribute, object value)
        {
            ...
            ushort[] arr = (ushort[])value;
            //id(1) + total length(4) + rank(4)
            memory = new byte[9 + Size.UInt16 * arr.Length];
            ...
            unsafe
            {
                fixed (ushort* pInt = arr)
                {
                    fixed (byte* pData = &memory[offset])
                    {
                        ushort* pTemp = pInt;
                        ushort* pArray = (ushort*)pData;
        ...
        {
            ushort[] ret;
            if (length == 4) return new ushort[0];
            unsafe
            {
                fixed (byte* pByte = &data[offset])
                {
                    int arrLength = *(int*)pByte;
                    ushort* pTemp = (ushort*)(pByte + 4);
                    ret = new ushort[arrLength];
