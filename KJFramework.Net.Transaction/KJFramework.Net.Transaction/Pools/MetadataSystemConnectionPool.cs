...
namespace KJFramework.Net.Transaction.Pools
{
    ...
    /// </summary>
    public class MetadataSystemConnectionPool : ConnectionPool<string, MetadataContainer>
    {
        #region Methods
        ...
        public IServerConnectionAgent<MetadataContainer> GetChannel(string key, string roleId, IProtocolStack<MetadataContainer> protocolStack, MetadataTransactionManager transactionManager)
        {
            try
            {
                string fullKey = string.Format("{0}#{1}", roleId, key);
                IServerConnectionAgent<MetadataContainer> agent = GetChannel(fullKey);
                if (agent != null)
                {
                ...
                agent = MetadataConnectionAgent.Create(iep, protocolStack, transactionManager);
        ...
        public override bool Add(string key, IServerConnectionAgent<MetadataContainer> channel)
        {
