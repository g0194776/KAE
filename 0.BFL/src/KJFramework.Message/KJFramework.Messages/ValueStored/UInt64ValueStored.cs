...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     UInt64类型的存储
    /// </summary>
    public class UInt64ValueStored : BaseValueStored
    {
        #region Members
        ...
        ///     UInt64类型存储的初始化构造器
        /// </summary>
        public UInt64ValueStored()
        {
            _typeId = (byte)PropertyTypes.UInt64;
        }

        /// <summary>
        ///     UInt64类型存储的初始化构造器
        /// </summary>
        public UInt64ValueStored(ulong value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.UInt64;
        }
        ...
        ///     UInt64类型存储的静态构造器
        /// </summary>
        static UInt64ValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<ulong>();
            _toBytesDelegate = ProcessorDictionary.ValueActions[(byte)PropertyTypes.UInt64];
            _toDataDelegate = ProcessorDictionary.DataActions[(byte)PropertyTypes.UInt64];
        }
