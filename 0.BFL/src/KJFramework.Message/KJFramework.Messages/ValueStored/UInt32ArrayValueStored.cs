...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     UInt32数组类型的存储
    /// </summary>
    public class UInt32ArrayValueStored : BaseValueStored
    {
        #region Members
        ...
        ///     UInt32数组类型存储的初始化构造器
        /// </summary>
        public UInt32ArrayValueStored()
        {
            _typeId = (byte)PropertyTypes.UInt32Array;
        }

        /// <summary>
        ///     UInt32数组类型存储的初始化构造器
        /// </summary>
        public UInt32ArrayValueStored(uint[] value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.UInt32Array;
        ...
        ///     UInt32数组类型存储的静态构造器
        /// </summary>
        static UInt32ArrayValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<uint[]>();
            _toBytesDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.UInt32Array)).ValueProcessor;
            _toDataDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.UInt32Array)).DataProcessor;
        }
        ...
        {
            return new UInt32ArrayValueStored();
        }
