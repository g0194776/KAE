...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     Guid数组类型的存储
    /// </summary>
    public class GuidArrayValueStored : BaseValueStored
    {
        #region Members

        private readonly Guid[] _value;
        private static readonly PropertyValueStored<Guid[]> _instance;
        ...
        ///     Guid数组类型存储的初始化构造器
        /// </summary>
        public GuidArrayValueStored()
        {
            _typeId = (byte)PropertyTypes.GuidArray;
        }

        /// <summary>
        ///     Guid数组类型存储的初始化构造器
        /// </summary>
        public GuidArrayValueStored(Guid[] value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.GuidArray;
        ...
        ///     Guid数组类型存储的静态构造器
        /// </summary>
        static GuidArrayValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<Guid[]>();
            _toBytesDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.GuidArray)).ValueProcessor;
            _toDataDelegate = (DataProcessorMapping.Instance.GetProcessor(PropertyTypes.GuidArray)).DataProcessor;
        }
        ...
        {
            return new GuidArrayValueStored();
        }
