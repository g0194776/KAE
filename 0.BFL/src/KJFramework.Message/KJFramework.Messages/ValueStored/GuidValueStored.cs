...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     Guid类型的存储
    /// </summary>
    public class GuidValueStored : BaseValueStored
    {
        #region Members
        ...
        ///     Guid类型存储的初始化构造器
        /// </summary>
        public GuidValueStored()
        {
            _typeId = (byte)PropertyTypes.Guid;
        }

        /// <summary>
        ///     Guid类型存储的初始化构造器
        /// </summary>
        public GuidValueStored(Guid value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.Guid;
        }
        ...
        ///     Guid类型存储的静态构造器
        /// </summary>
        static GuidValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<Guid>();
            _toBytesDelegate = ProcessorDictionary.ValueActions[(byte)PropertyTypes.Guid];
            _toDataDelegate = ProcessorDictionary.DataActions[(byte)PropertyTypes.Guid];
        }
