...
namespace KJFramework.Messages.ValueStored.DataProcessor
{
    ...
    ///     SByte数组关于元数据的处理
    /// </summary>
    public class SByteArrayDataProcessor : IDataProcessor
    {
        #region Members
        ...
        ///     获取当前处理的第三方数据类型
        /// </summary>
        public PropertyTypes TypeId { get { return PropertyTypes.SByteArray; } }
        ...
        public unsafe void DataProcessor(MetadataContainer metadataObject, byte id, byte[] data, int offsetStart, uint length)
        {
            ...
            if (length == 0)
            {
                metadataObject.SetAttribute(id, new SByteArrayValueStored(new sbyte[0]));
                return;
            }
            sbyte[] array = new sbyte[length / Size.SByte];
            fixed (byte* pByte = (&data[offsetStart]))
            {
                sbyte* pData = (sbyte*)pByte;
                for (int j = 0; j < array.Length; j++) array[j] = *pData++;
            }
            metadataObject.SetAttribute(id, new SByteArrayValueStored(array));
        }
        ...
        public unsafe void ValueProcessor(IMemorySegmentProxy proxy, BaseValueStored baseValueMessage)
        {
            ...
            fixed (sbyte* pByte = baseValueMessage.GetValue<sbyte[]>())
                proxy.WriteMemory(pByte, (uint)baseValueMessage.GetValue<sbyte[]>().Length * Size.SByte);
        }
