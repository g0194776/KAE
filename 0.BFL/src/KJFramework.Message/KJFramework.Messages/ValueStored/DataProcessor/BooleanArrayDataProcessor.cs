...
namespace KJFramework.Messages.ValueStored.DataProcessor
{
    ...
    ///     Boolean数组关于元数据的处理
    /// </summary>
    public class BooleanArrayDataProcessor : IDataProcessor
    {
        #region Members
        ...
        ///     获取当前处理的第三方数据类型
        /// </summary>
        public PropertyTypes TypeId { get { return PropertyTypes.BooleanArray; } }
        ...
        public unsafe void DataProcessor(MetadataContainer metadataObject, byte id, byte[] data, int offsetStart, uint length)
        {
            ...
            if (length == 0)
            {
                metadataObject.SetAttribute(id, new BooleanArrayValueStored(new bool[0]));
                return;
            }
            bool[] array = new bool[length / Size.Bool];
            fixed (byte* pByte = (&data[offsetStart]))
            {
                bool* pData = (bool*)pByte;
                for (int j = 0; j < array.Length; j++) array[j] = *pData++;
            }
            metadataObject.SetAttribute(id, new BooleanArrayValueStored(array));
        }
        ...
        public unsafe void ValueProcessor(IMemorySegmentProxy proxy, BaseValueStored baseValueMessage)
        {
            ...
            fixed (bool* pByte = baseValueMessage.GetValue<bool[]>())
                proxy.WriteMemory(pByte, (uint)baseValueMessage.GetValue<bool[]>().Length * Size.Bool);
        }
