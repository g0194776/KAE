...
namespace KJFramework.Messages.ValueStored.DataProcessor
{
    ...
    ///     Int16数组关于元数据的处理
    /// </summary>
    public class Int16ArrayDataProcessor : IDataProcessor
    {
        #region Members
        ...
        ///     获取当前处理的第三方数据类型
        /// </summary>
        public PropertyTypes TypeId { get { return PropertyTypes.Int16Array; } }
        ...
        public unsafe void DataProcessor(MetadataContainer metadataObject, byte id, byte[] data, int offsetStart, uint length)
        {
            ...
            if (length == 0)
            {
                metadataObject.SetAttribute(id, new Int16ArrayValueStored(new short[0]));
                return;
            }
            short[] array = new short[length / Size.Int16];
            if (array.Length <= 10)
            {
            ...
            metadataObject.SetAttribute(id, new Int16ArrayValueStored(array));
        }
        ...
        public unsafe void ValueProcessor(IMemorySegmentProxy proxy, BaseValueStored baseValueMessage)
        {
            ...
            fixed (short* pByte = baseValueMessage.GetValue<short[]>())
                proxy.WriteMemory(pByte, (uint)baseValueMessage.GetValue<short[]>().Length * Size.Int16);
        }
