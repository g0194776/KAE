...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     IntPtr类型的存储
    /// </summary>
    public class IntPtrValueStored : BaseValueStored
    {
        #region Members

        private readonly IntPtr _value;
        private static readonly PropertyValueStored<IntPtr> _instance;
        ...
        ///     IntPtr类型存储的初始化构造器
        /// </summary>
        public IntPtrValueStored()
        {
            _typeId = (byte)PropertyTypes.IntPtr;
        }

        /// <summary>
        ///     IntPtr类型存储的初始化构造器
        /// </summary>
        public IntPtrValueStored(IntPtr value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.IntPtr;
        }
        ...
        ///     IntPtr类型存储的静态构造器
        /// </summary>
        static IntPtrValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<IntPtr>();
            _toBytesDelegate = ProcessorDictionary.ValueActions[(byte)PropertyTypes.IntPtr];
            _toDataDelegate = ProcessorDictionary.DataActions[(byte)PropertyTypes.IntPtr];
        }
