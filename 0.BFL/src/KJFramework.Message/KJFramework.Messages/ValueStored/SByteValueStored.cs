...
namespace KJFramework.Messages.ValueStored
{
    ...
    ///     SByte类型的存储
    /// </summary>
    public class SByteValueStored : BaseValueStored
    {
        #region Members
        ...
        ///     SByte类型存储的初始化构造器
        /// </summary>
        public SByteValueStored()
        {
            _typeId = (byte)PropertyTypes.SByte;
        }

        /// <summary>
        ///     SByte类型存储的初始化构造器
        /// </summary>
        public SByteValueStored(sbyte value)
        {
            _value = value;
            _typeId = (byte) PropertyTypes.SByte;
        }
        ...
        ///     Byte类型存储的静态构造器
        /// </summary>
        static SByteValueStored()
        {
            _instance = ValueStoredHelper.BuildMethod<sbyte>();
            _toBytesDelegate = ProcessorDictionary.ValueActions[(byte)PropertyTypes.SByte];
            _toDataDelegate = ProcessorDictionary.DataActions[(byte)PropertyTypes.SByte];
        }
