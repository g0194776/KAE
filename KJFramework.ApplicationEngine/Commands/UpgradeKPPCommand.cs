...
namespace KJFramework.ApplicationEngine.Commands
{
    ...
    ///     更新指定已上架KPP到KAE宿主的命令
    /// </summary>
    internal class UpgradeKPPCommand : IKAESystemCommand
    {
        #region Members.
        ...
        public MessageIdentity SupportedCommand
        {
            get
            {
                return new MessageIdentity {ProtocolId = 0xFF, ServiceId = 0x00, DetailsId = 0x04};
            }
        }
        ...
        public IExecuteResult Execute(MetadataContainer msg, KAEHost host, IKAEHostAppManager hostedAppManager, IKAEStateLogger stateLogger)
        {
            ...
            //installing KPP.
            kppUniqueId = msg.GetAttributeAsType<Guid>(0x0B);
            if (kppUniqueId == Guid.Empty) return ExecuteResult.Fail((byte)KAEErrorCodes.IllegalArgument, string.Empty);
            ...
            app = new ApplicationDynamicObject(kppInfo.Item2, kppInfo.Item3, host.ChannelInternalConfigSettings, host.ResourceProxy, host.HandleSucceedSituation, host.HandleErrorSituation);
